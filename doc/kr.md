...
```
```txt
字符串常量被编译到可执行文件中，通常保存在.rdata只读节区
通过vc6查看字符串指针的值,减去0x00400000等于字符串在文件中的偏移量, 可通过winhex通过偏移量查找

.text 用来存放程序中的可执行代码部分，包括我们在 C 代码中定义的函数和运行时函数等。
.rdata 是指 readable data，用来存放只读数据，包括字符串、const 常量、导入导出的函数等。
.data 是可读和可写的数据，用来存放 global 和 static 类型的变量和数据结构。
.rsrc 用来存放资源部分，比如 GUI 程序中的各种控件的位置大小和形状、图片、背景音乐等。
.reloc 用来存放重定位表。
```
```txt
IDA快捷键
在变量上按a, 可转换为原始字符串
在16进制数上按b，可转换为二进制数，可按shift加-可转换为真值
```
